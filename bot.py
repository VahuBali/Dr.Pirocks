...
async def youtube(ctx):
    await ctx.send(
        "Vasu Bansal's YouTube channel can be found at https://www.youtube.com/channel/UCCzfHZSA8nEnx_QTFlq6mHA")


@client.command()
async def podcast(ctx):
    await ctx.send(
        "Vasu Bansal's Podcast can be found at https://redcircle.com/talk-tech-teen-tech or on Spotify, Apple Podcasts, or anywhere you listen to your podcasts!")


@client.command()
async def joindis(ctx):
    await ctx.send(
        "Join the vbYT discord where you can also ask any questions about this bot using this link https://discord.gg/npzKUsG9PS")


@client.command()
async def inv(ctx):
    await ctx.send(
        "Get the Admins to click this link to add this bot https://discord.com/api/oauth2/authorize?client_id=788895628511281182&permissions=8&scope=bot")
...
async def beg(ctx):
    ...
    await ctx.send(
        f"**Dr.Pirocks gave you {earnings} moolah now use this money to do something good for the planet!!**")
